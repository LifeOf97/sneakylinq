...
def format_alias(device: str, alias: str) -> tuple[str, str, bool]:
    ...
    User's alias are stored in redis as hash, with name "device:alias" using
    device:did as key and the alias as value.

    :did: user's did as key
    :alias: the new alias to set
    """
    _device_alias: str = "device:alias"
    # _alias_device: str = "alias:device"
    alias: str = f"{alias}.linq"

    message: str = "Alias accepted"
    status: bool = True

    if alias == redis_client.hget(_device_alias, key=device):
        status = False
        message = f"{alias} is already your device alias"

    elif alias in redis_client.hvals(_device_alias):
        status = False
